...
class MultilayerPerceptron(torch.nn.Module):
    ...
    def forward(self, x):
        # embedded = self.embedding(residue)
...
# model.eval()
# train_acc = compute_accuracy(model, train_loader, device=DEVICE, with_embedding=False, which_model='categorical')

# train_mae, train_mse = compute_mae_and_mse(model, train_loader, device=DEVICE,
#                                            with_embedding=False, which_model='categorical')
...
# model.eval()
# test_acc = compute_accuracy(model, test_loader, device=DEVICE, with_embedding=False,which_model='categorical')
...
# true_labels, predicted_labels = get_labels_and_predictions(
#     which_model='categorical',
#     model=model, data_loader=test_loader, device=torch.device('cpu'), with_embedding=False)
